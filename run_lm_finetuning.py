...
class TextDataset(Dataset):
    ...
    def __init__(self, tokenizer, args, file_path='train', block_size=512, train=True, max_q_length= None, max_a_length = None):
            ...
            with open(file_path, encoding="utf-8") as f:
                ...
                for line in tokenized_lines:
                    ...
                    if len(q)<self.max_q_len:
                        for _ in range(len(q), self.max_q_len, 1):
                            q.append(tokenizer.convert_tokens_to_ids('<PAD>'))
                    if len(a) < self.max_a_len:
                        for _ in range(len(a), self.max_a_len, 1):
                            a.append(tokenizer.convert_tokens_to_ids('<PAD>'))
    ...
    def get_max_length(self, lines, tokenizer):
        max_q_len = -1
        max_a_len = -1
        result = []
        for line in lines:
...
def load_and_cache_examples(args, tokenizer, evaluate=False, max_q = None, max_a = None):
    dataset = TextDataset(tokenizer, args, file_path=args.eval_data_file if evaluate else args.train_data_file,
                          block_size=args.block_size, train = not evaluate, max_q_length=max_q, max_a_length=max_a)
    return dataset
...
def evaluate(args, model, tokenizer, prefix="", max_q = None, max_a = None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        batch_labels = batch
        batch_labels = torch.where(batch_labels==tokenizer.convert_tokens_to_ids('<PAD>'),torch.ones_like(batch).fill_(-1), batch_labels)
        batch_labels[:, :eval_dataset.max_q_len] = -1
        inputs, labels = mask_tokens(batch, tokenizer, args) if args.mlm else (batch_inputs, batch_labels)
        inputs = inputs.to(args.device)
        labels = labels.to(args.device)
...
def main():
    ...
    print('block size', args.block_size, tokenizer.max_len_single_sentence)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, max_q=train_dataset.max_q_len, max_a=train_dataset.max_a_len)
